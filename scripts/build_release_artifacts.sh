...
PUBLIC_LATEST_BASE_URL="${PUBLIC_LATEST_BASE_URL:-}"
...
if [[ -z "${PUBLIC_LATEST_BASE_URL}" && -n "${GITHUB_REPOSITORY:-}" ]]; then
  # 关键逻辑：AltStore source 始终指向 latest，避免固定页面里的 downloadURL 滞后到旧 tag。
  PUBLIC_LATEST_BASE_URL="https://github.com/${GITHUB_REPOSITORY}/releases/latest/download"
fi
if [[ -z "${PUBLIC_RELEASE_BASE_URL}" ]]; then
  PUBLIC_RELEASE_BASE_URL="https://example.com/${APP_NAME}/${RELEASE_TAG}"
fi
if [[ -z "${PUBLIC_LATEST_BASE_URL}" ]]; then
  PUBLIC_LATEST_BASE_URL="${PUBLIC_RELEASE_BASE_URL}"
fi
...
IPA_PUBLIC_LATEST_URL="${PUBLIC_LATEST_BASE_URL}/${APP_NAME}.ipa"
      ...
      "versions": [
        {
          ...
          "date": "${VERSION_DATE}",
          "downloadURL": "${IPA_PUBLIC_LATEST_URL}",
          "size": ${IPA_SIZE_BYTES},
          "minOSVersion": "17.0"
        }
      ]
...
<body>
  <main class="wrap">
    <section class="card">
      ...
      <a class="btn" href="${IPA_PUBLIC_LATEST_URL}">下载 IPA（外网）</a>
...
cat >"${OUTPUT_DIR}/build-meta.json" <<JSON
{
  ...
  "build_time_utc": "${VERSION_DATE_UTC}",
  "time_zone": "${TIME_ZONE}",
  "version": "${APP_VERSION}",
  ...
  "artifact_urls": {
    ...
    "public_latest_base_url": "${PUBLIC_LATEST_BASE_URL}",
    "ipa": "${IPA_PUBLIC_URL}",
    "ipa_latest": "${IPA_PUBLIC_LATEST_URL}",
